...
_C_SURFACE = "#FFFFFF"       # 요약 밴드 metric 표면 (UI/UX 3C) · 카드 표면 (3D)
_C_BORDER = "#E5E7EB"        # 요약 밴드·내비게이션 얇은 테두리 (UI/UX 3C)
_C_CARD_BORDER = "#E2E8F0"   # 섹터·매매 카드 표면 테두리 (UI/UX 3D)
# 상승·하락·손익 시맨틱색 (한국 관례: 양수=빨강 / 음수=파랑 / 0·없음=중립).
# 색만으로 의미를 전달하지 않도록 항상 +/− 부호 텍스트와 병행한다.
_C_UP = "#DC2626"            # 양수·상승·수익 (UI/UX 3D)
_C_DOWN = "#2563EB"          # 음수·하락·손실 (UI/UX 3D)
_C_WARN_ACCENT = "#F59E0B"   # 카드 내부 앰버 경고 영역 좌측 강조선 (UI/UX 3D)
# 앰버 경고 영역 배경/글자는 _BADGE_AMBER(#FEF3C7/#92400E)를 재사용한다(아래 정의).
...
# ── 상승·하락·손익 표현 헬퍼 (UI/UX 3D · 순수 함수) ──────────
# 한국 관례: 양수=빨강+'+' / 음수=파랑+'-' / 0·없음=중립(부호 없음).
# 기존 계산 결과를 그대로 표시만 바꾼다 — 반올림·기준값·데이터 소스는 변경하지 않는다.
def signed_pct_text(value, digits: int = 1):
    """부호 병행 퍼센트 텍스트. 양수 '+4.0%' · 음수 '-11.2%'(부호 중복 없음) ·
    0 '0.0%'(부호 없음) · 값 없음/이상 None. 반올림은 기존과 동일한 소수 1자리."""
    try:
        if value is None or pd.isna(value):
            return None
        v = float(value)
        ...
        return None
    if v == 0:
        return f"{0:.{digits}f}%"        # 0 → 중립(부호 없음)
    return f"{v:+.{digits}f}%"           # 파이썬 '+.1f'는 음수에 '-'를 한 번만 붙임


def semantic_sign_color(value) -> str:
    """부호에 따른 시맨틱색: 양수=_C_UP(빨강) · 음수=_C_DOWN(파랑) · 0/없음=중립.
    색은 부호 텍스트를 보조할 뿐 색만으로 의미를 전달하지 않는다."""
    try:
        if value is None or pd.isna(value):
            return _C_TEXT_MUTED
        v = float(value)
    except (TypeError, ValueError):
        return _C_TEXT_MUTED
    if v > 0:
...
def signed_amount_text(value):
    """부호 병행 금액 텍스트(실현 손익용). 양수 '+90,000' · 음수 '-30,000'(중복 없음) ·
    0 '0'(부호 없음) · 값 없음 '—'. 정수는 소수 없이, 소수값은 자릿수를 보존한다."""
    try:
        if value is None or pd.isna(value):
            return "—"
        v = float(value)
        ...
        return "—"
    body = f"{abs(v):,.0f}" if v == int(v) else f"{abs(v):,}"
    if v > 0:
        return f"+{body}"
    if v < 0:
        return f"-{body}"
    return body                          # 0 → 부호 없음(중립)


def _pct_span(value, digits: int = 1) -> str:
    """부호+시맨틱색을 입힌 인라인 HTML span. 값 없으면 빈 문자열."""
    txt = signed_pct_text(value, digits)
    if txt is None:
        return ""
    return f"<span style='color:{semantic_sign_color(value)};'>{txt}</span>"


def _high_line_html(row) -> str:
    """섹터 카드 52주 고점 줄(HTML): 순수 텍스트 format_52w_high_line을 재사용하고
    '고점 대비 X%' 부분만 시맨틱색 span으로 치환한다. USD '$'는 평문 유지(escape)."""
    line = html.escape(format_52w_high_line(row)).replace("$", "\\$")
    gap = calc_gap_from_high(row.get("close"), row.get("high_52w"))
    if gap is not None:
        line = line.replace(f"고점 대비 {gap:+.1f}%", f"고점 대비 {_pct_span(gap)}")
    return line


def _trade_line_html(r: dict, currency: str, current_price=None):
    """섹터 카드 매매 연동 줄(HTML): 순수 텍스트 format_trade_line을 재사용하고
    '현재가 대비 X%' 부분만 시맨틱색 span으로 치환한다. USD '$'는 평문 유지(escape).
    목표가·진입가·손절가 숫자 자체는 색칠하지 않는다(등락·이격 값만 색)."""
    line = format_trade_line(r, currency, current_price)
    if line is None:
        return None
    line = line.replace("$", "\\$")
    _, price = trade_display_price(r)
    gap = gap_vs_current(price, current_price)
    if gap is not None:
        line = line.replace(f"현재가 대비 {gap:+.1f}%", f"현재가 대비 {_pct_span(gap)}")
    return line


def _warn_box(msg: str) -> str:
    """카드 내부 앰버 경고 영역(인라인 style). 옅은 앰버 배경 + 좌측 강조선.
    경고 문구 내용은 그대로 두고 시각 표현만 회색 caption → 앰버 영역으로 바꾼다."""
    txt = html.escape(str(msg)).replace("$", "\\$")
    return (f"<div style='background:{_BADGE_AMBER[0]};color:{_BADGE_AMBER[1]};"
            f"border-left:3px solid {_C_WARN_ACCENT};border-radius:6px;"
            f"padding:6px 10px;font-size:12px;line-height:1.4;margin:2px 0 4px;'>{txt}</div>")


def _pnl_sign_key(value) -> str:
    """실현 손익 부호 분류 키 조각: pos/neg/flat/na. 시맨틱색 CSS 적용 범위 한정용."""
    try:
        if value is None or pd.isna(value):
            return "na"
        v = float(value)
...
def _pnl_metric(col, label: str, value, rec_id, slot: str = "card"):
    """실현 손익 metric: 부호 병행 텍스트 + 부호별 container key로 시맨틱색을 입힌다.
    st.metric을 그대로 쓰므로 모바일 카드 압축 CSS(metric 크기)도 동일하게 적용된다.
    값 없음(na)은 색 없이 '—'만 표시(빈 강조 렌더 방지)."""
    k = _pnl_sign_key(value)
    if k == "na":
        col.metric(label, "—")
        return
    with col.container(key=f"tr_pnl{k}_{slot}_{rec_id}"):
        st.metric(label, signed_amount_text(value))


# ── 카드 표면·손익색 CSS (UI/UX 3D) ─────────────────────────
# 적용 범위: 섹터 카드(st-key-stock_card_*)·매매 카드(st-key-trade_card_*) 표면과,
# 실현 손익 metric을 감싼 부호별 key(st-key-tr_pnlpos/neg/flat_*)만. 클래스·data-testid
# 속성 선택자만 사용 — :has·nth-child·first/last-child·전역 stMetric 선택자 없음.
# 기존 _MOBILE_CARD_CSS(모바일 압축)·_HEADER_NAV_CSS(3C)는 변경하지 않는다.
_CARD_V3D_CSS = f"""<style>
/* 카드 표면 정돈 — 흰 배경·얇은 테두리·둥근 모서리·절제된 그림자 (카드 key 한정) */
[class*="st-key-stock_card_"], [class*="st-key-trade_card_"] {{
...
/* 실현 손익 시맨틱색 — 완료 총 손익 metric 값에만(부호별 key 범위 한정) */
[class*="st-key-tr_pnlpos_"] [data-testid="stMetricValue"] {{ color: {_C_UP}; }}
[class*="st-key-tr_pnlneg_"] [data-testid="stMetricValue"] {{ color: {_C_DOWN}; }}
[class*="st-key-tr_pnlflat_"] [data-testid="stMetricValue"] {{ color: {_C_TEXT_MUTED}; }}
</style>"""
...
def render_stock_card(row: dict, keyns: str = "map"):
    ...
    with st.container(border=True, key=f"stock_card_{keyns}_{code}"):
        ...
        # 52주 고점(보조) + 고점 대비 이격률(시맨틱색·부호 병행). 캡션 노드 유지(모바일 CSS 적용).
        left.caption(_high_line_html(row), unsafe_allow_html=True)
        ...
        if link_lines:
            ...
            for ln in link_lines:
                right.caption(ln, unsafe_allow_html=True)
        else:
...
def _toss_overlay_state() -> dict:
    """현재 rerun의 Toss overlay(session_state). 비활성이면 key 자체가 없어 {} —
    호출측은 빈 overlay일 때 Toss 분기를 타지 않고 기존 DB 경로로 간다."""
    try:
...
def _apply_toss_overlay(records):
    """(활성 전용 — _maybe_apply_toss_overlay 게이트 뒤에서만 호출)
...
def trade_link_lines(row, currency: str, max_lines: int = 3) -> list[str]:
    ...
    for r in recs[:max_lines]:
        line = _trade_line_html(r, currency, row.get("close"))   # 현재가 대비 이격률에 시맨틱색
        if line:
            lines.append(line)
...
def _trade_calc(r: dict):
    ...
    # 활성 Toss overlay가 있을 때만 Toss 분기를 탄다 — 비활성(빈 overlay)이면
    # 기존 3997bb6과 동일한 _ext_quote → DB 경로만 실행(카드별 Toss helper 호출 0).
    ...
    if lev_sym:
        ...
        if isinstance(ext, qt.QuotePair):
            ...
            pair = (tov.pick_pair(ov, base_s, etf_s, max_skew_sec=TOSS_MAX_SKEW_SEC)
                    or db_quote_pair(base_s, etf_s))
        else:
            pair = db_quote_pair(base_s, etf_s)
        ...
        if isinstance(ext, qt.QuoteSnapshot):
            ...
            snap = tov.pick_single(ov, base_s) or db_single_quote(base_s)
        else:
            snap = db_single_quote(base_s)
