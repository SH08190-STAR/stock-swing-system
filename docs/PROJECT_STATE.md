...
## 최신 저장소 커밋 (main HEAD)
- `b0219ab` merge: add Cloud module reload guard (2026-07-13 push)
- 앱 코드(dashboard/app.py·app.database·app.quotes) 변경 포함 배포이며
  운영 검증까지 완료돼 아래 LKG와 동일하다.

## LAST_KNOWN_GOOD_COMMIT
- `b0219ab` merge: add Cloud module reload guard (2026-07-13 갱신)
- 의미: **운영(Streamlit Cloud)에서 정상 동작이 확인된 커밋.** 현재 main HEAD와 동일.
- 근거: ETF quote-pair v2 병합(bc2055a) 후 발생한 부분 hot-reload AttributeError를
  런타임 모듈 정합성 가드로 해소(b0219ab). 2026-07-13 tests·Deploy Smoke workflow 성공 +
  운영 앱 실화면 확인(로그인·매매기록 탭·본주/ETF 카드·기준일 07-10·환산값·AttributeError 없음·
  모듈 동기화 실패 없음).
- 이전 LKG `9975719`에서 갱신: 위 조건(앱 배포 + smoke 통과 + 운영 실화면 확인) 충족.
