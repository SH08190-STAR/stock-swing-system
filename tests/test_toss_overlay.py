...
# ── 1~3. credentials 비활성 경로(네트워크 0회) ──────────────
def test_dashboard_disabled_when_no_creds(monkeypatch):
    m = _dash()
    m._toss_client.clear()               # cache_resource는 _dash() 재로드 간 공유 → 명시적 초기화
    monkeypatch.setattr(m.config, "TOSS_CLIENT_ID", "")
    monkeypatch.setattr(m.config, "TOSS_CLIENT_SECRET", "")
    assert m._toss_client() is None
    assert m.toss_enabled() is False
    # 비활성이면 fetch도 네트워크 0회(client None)
    prices, err = m._fetch_toss_prices(None, ["005930"])
...
def test_dashboard_disabled_with_one_cred(monkeypatch):
    ...
    monkeypatch.setattr(m.config, "TOSS_CLIENT_SECRET", "")
    assert m._toss_client() is None and m.toss_enabled() is False
